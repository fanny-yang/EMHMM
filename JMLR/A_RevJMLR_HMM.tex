...
using $\kdim = \numobs$.
%, which allows for a better approximation than
%for $\kdim < \numobs$ but 
...
in the stationary distribution, $\mixcoefeps$ is the mixing
constant from equation~\eqref{ass:mixing}, and $c(\cdot)$ is
a constant dependent only on the inherent model parameters. 
...
Lipschitz gradients as well, it is actually of a completely
different nature. The important difference arises from the
existence of two parameters. 

 As opposed to gradient descent, the EM updates optimize over the
 first parameter $\paramjoint$ of a function
 $\qfunpoptrunc{\cdot}{\paramjoint'}$ defined by the second parameter
 $\paramjoint'$ at every time step. If we could access
 $\qfunpoptrunc{\cdot}{\trueparamjoint}$, EM would converge in one
 step to the true optimum.  Therefore, if we can guarantee that
 $\qfunpoptrunc{\cdot}{\paramjoint'}$ and
 $\qfunpoptrunc{\cdot}{\trueparamjoint}$ are close in some sense,
 there should be good reasons to hope that under some more regularity
 assumptions the maximizers are close
 as well, i.e.  that $\emoppoptrunc{\paramjoint'}$ is close to
 $\trueparamjoint$.
...
In particular, we
require the gradients (with respect to the first argument
$\paramjoint$) to be Lipschitz in the second argument $\paramjoint'$.
Typical smoothness however 
 is a property with respect to a
\emph{fixed} function (i.e. a fixed $\paramjoint'$ in our case) and thus
requires gradients to be Lipschitz in the first argument. Loosely
speaking it upper bounds the curvature of said function, and thus is more
like a second-order condition by nature.

This distinction also also explains why $(L_{\paramobs},
L_{\paramtrans})$-FOS conditions require to be uniformly 
satisfied only over the first
argument, while one of the second arguments can be fixed
at $\trueparamjoint$.
\mycomment{Not quite so successful attempt to address one reviewers
confusion about Lipschitz v.s our condition}
%require it to hold with respect to parameter $\trueparamjoint$

%, in the form of
%their gradients with respect to the first argument. 
%% Intuitively, this together with strong
%% concavity makes sure that if the current iterate $\paramjoint^t$ is
%% close to $\trueparamjoint$, then the EM update finds a new iterate
%% $\paramjoint^{t+1}$ which is close to the maximizer of $\qfunpoptrunc{\cdot}
%% {\trueparamjoint}$ (which is by self-consistency $\trueparamjoint$
%% itself).. 
As we
show in Section~\ref{sec:normal_HMM}, these conditions hold for
concrete models.
...
is a good choice to obtain a good trade-off. 
...
is lower bounded as \mbox{$\numobs \geq
  \frac{\BIGCON_1}{(1-2\kappa)\|\trueparamobs\|_2^2\sigma^2} (\SNR +
...
( \log\frac{1+b}{1-b} - \log \|\mustar\|_2)$, with
...
% 
&\stackrel{\mathrm{(i)}}{\geq}  (1 - 4 \kdim) \: \mprob( \sup_{g\in\mathcal{F}}\| g^{H^1}(X) - \EE
g(X)\|_2 \geq \epsilon),
\end{eqnarray*}
...
%% and
%% moreover, Lemma~\ref{lem:twosided_approx} states that
%% \begin{align*}
%% \sup_{\paramjointone,\paramjointtwo \in \DomTheta} \sup_{x} \sum_{z_i}
%% \mid p(z_i \mid x_{1}^{\subsize},\theta') - p(z_i \mid
%% x_{i-k}^{i+k},\paramjointtwo)| \leq \frac{ C\nstates^2 \big(1-\mixcoefeps
%%   \statmin \big)^{\min\{i, \numobs -i, \kdim\}}}{\mixcoefeps^9 \statmin^{2}}.
%% \end{align*}
...
% a $\chi$-squared variable of degree $m$ with mean $\sigma^2$, thus
...
%%   -  1\right|\frac{p(x_{i+1}^{\blockrightind})}{p(x_{i+1}^{\blockrightind}|x_{\blockleftind}^{i})}
%%   \frac{1}{p(z_i)}.
%% \end{align*}
%% %
%% From Lemma~\ref{lem:filterstab} we directly obtain the following upper bounds
...
%% \end{align*}
%% and $\frac{p(x_{i+1}^{\blockrightind})}{p(x_{i+1}^{\blockrightind}|x_{\blockleftind}^{i})}
...
%%   \frac{1}{p(z_i)}\\
%% %
%% \leq &\left( \frac{|p(x_{i+k+1}^{n}|x_{i+1}^{i+k}) -
%%   p(x_{i+k+1}^{n}|x_{1}^{i+k})|}{p(x_{i+k+1}^{n}|x_{1}^{i+k})}
...
%%   \mid x_{\blockleftind}^{\blockrightind}) } \right) \frac{\nstates \mixcoefeps^{-2}}{\statmin}\\
%% %
%% \leq &\frac{C \nstates^2
%%   \mixcoefeff^k}{\mixcoefeps^8 \statmin}
%% \end{align*}
